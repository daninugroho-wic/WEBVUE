...
<div class="flex h-screen">
    ...
    <section class="flex flex-col flex-1 bg-green-50">
        <main class="flex-1 p-4 overflow-y-auto">
            <div v-for="(msg, idx) in messages" :key="idx" class="mb-2 flex" :class="msg.sender === 'me' ? 'justify-end' : 'justify-start'">
                <div class="relative max-w-xs p-2 rounded-lg text-sm text-black" :class="msg.sender === 'me' ? 'bg-green-600 text-white' : 'bg-green-200'">
                    <div>{{ msg.text }}</div>
...
<template>
    <div class="h-full w-full bg-white shadow-lg p-4">
        ...
        <!-- Daftar Kontak -->
        <div v-for="(contact, index) in contacts" :key="index" class="flex items-center justify-between gap-2 p-2 hover:bg-green-100 border rounded mb-1 cursor-pointer">
